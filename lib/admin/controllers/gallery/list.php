...
class Vimeography_Gallery_List extends Vimeography_Base
{
  ...
  {
    add_action('vimeography_action_duplicate_gallery', array(
      $this,
      'duplicate_gallery'
    ));
    add_action('vimeography_action_delete_gallery', array(
      $this,
      'delete_gallery'
    ));
    add_action('vimeography_action_bulk_process_galleries', array(
      $this,
      'bulk_process'
    ));
  ...
  public function duplicate_gallery($params)
  {
    if (
      ...
      if (
        isset($params['vimeography_duplicate_gallery_serialized']) and
        !empty($params['vimeography_duplicate_gallery_serialized'])
      ) {
        $params = unserialize(
          stripslashes($params['vimeography_duplicate_gallery_serialized'])
        );
      }

      if (
        isset($params['duplicate_appearance']) &&
      ...
      try {
        ...
        $resource_uri = Vimeography::validate_vimeo_source(
          $params['gallery_source']
        );
        ...
        } elseif (isset($filesystem_connection)) {
  ...
  {
    if (
      $this->_table->current_action() == 'delete' and
      isset($input['gallery'])
    ) {
