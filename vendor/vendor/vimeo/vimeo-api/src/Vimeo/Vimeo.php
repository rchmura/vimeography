...
if (!function_exists('json_decode')) {
  throw new \Exception(
    'We could not find json_decode. json_decode is found in php 5.2 and up, but may be missing on some Linux systems due to licensing conflicts. If you are running ubuntu try "sudo apt-get install php5-json".'
  );
}

if (!function_exists('curl_init')) {
  throw new \Exception(
    'Could not find the cURL PHP extension. If you are running ubuntu try "sudo apt-get install php5-curl" and restart your server. If you need help, contact your hosting provider for support.'
  );
}
...
class Vimeo
{
  ...
  public function __construct($client_id, $client_secret, $access_token = null)
  {
    ...
    $this->CURL_DEFAULTS = apply_filters(
      'vimeography.request.curl_defaults',
      array(
        CURLOPT_HEADER => 1,
  ...
  public function uploadImage($pictures_uri, $file_path, $activate = false)
  {
    ...
    if ($pictures_response['status'] !== 201) {
      throw new VimeoUploadException(
        'Unable to request an upload url from vimeo'
      );
    }
    ...
    if ($activate) {
      $completion = $this->request(
        $pictures_response['body']['uri'],
        array('active' => true),
        'PATCH'
      );
    }
   ...
   */
  public function uploadTexttrack(
    $texttracks_uri,
    $file_path,
    $track_type,
    $language
    ...
    $texttrack_response = $this->request(
      $texttracks_uri,
      array('type' => $track_type, 'language' => $language, 'name' => $name),
      'POST'
    );
  ...
  private function _request($url, $curl_opts = array())
  {
    ...
    if (isset($curl_info['http_code']) && $curl_info['http_code'] === 0) {
      ...
      throw new VimeoRequestException(
        'Unable to complete request.' . $curl_error
      );
    }
    ...
    if (
      false !==
      stripos($response, "HTTP/1.0 200 Connection established\r\n\r\n")
    ) {
  ...
  private function perform_upload($file_path, $ticket)
  {
    if ($ticket['status'] !== 201) {
      $ticket_error = !empty($ticket['body']['error'])
        ? '[' . $ticket['body']['error'] . ']'
        : '';
      throw new VimeoUploadException(
        'Unable to get an upload ticket.' . $ticket_error
      );
    }
    ...
    do {
      ...
      $curl_opts[CURLOPT_HTTPHEADER][] =
        'Content-Range: bytes ' . $server_at . '-' . $size . '/' . $size;
    ...
    if ($status !== 201) {
      $error = !empty($completion['body']['error'])
